...
def save_files(sentence_train_list,sentence_test_list,sentence_validation_list):
    ...
    with open(save_path+"train_"+str(train_data_count)+".txt",'w',encoding='utf-8') as f:
        for sentence in sentence_train_list:
            f.write(sentence+"\n")
    with open(save_path+"test_"+str(test_data_count)+".txt",'w',encoding='utf-8') as f:
        for sentence in sentence_test_list:
            f.write(sentence+"\n")
    with open(save_path+"validation_"+str(validation_data_count)+".txt",'w',encoding='utf-8') as f:
...
def check_syntactic_sentence(ori_syntactic_sentence,accepted_pos_list):
    ...
    if len(doc)!=len(syntactic_token_list) and len(syntactic_token_list)!=0:
        return True
    return False
